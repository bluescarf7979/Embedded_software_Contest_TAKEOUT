...
def forecast(msg):
    ...
    authKey = keys
    url = 'https://apihub.kma.go.kr/api/typ01/url/kma_sfctm2.php?tm='+str(tm)+'&stn='+'108'+'&authKey='+str(authKey)  # API URL 설정
    print(url)
    ...
    for i in range (15,22,1):
        if float(result[i])<0:
            result[i]=0
    SD_HR3,SD_DAY,SD_TOT=float(result[11]),float(result[16]),float(result[21])
    # 응답 출력
